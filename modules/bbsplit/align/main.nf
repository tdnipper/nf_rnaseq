process bbsplit_align {
    ...
    output:
    tuple val(sample), path("*_nonmyco.fastq.gz"), emit: decon_reads
    path "*.txt"

    script:
    """
    bbsplit.sh in=${reads[0]} in2=${reads[1]} basename=${sample}_#_%.fastq.gz refstats=${sample}_stats.txt -Xmx36g threads=6
    """
}
